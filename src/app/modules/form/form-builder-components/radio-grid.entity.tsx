...
export const radioGridFieldEntity = createEntity({
  name: 'radioGridField',
  ...
  validate(values, context) {
    ...
    >(keys: K, values: V, required: boolean = false) {
      const shape = Object.fromEntries(keys.map(k => [k, required ? z.enum(values) : z.enum(values).optional()]));
      return z.object(shape as unknown as { [P in K[number]]: z.ZodEnum<V> });
    }
...
export const RadioGridFieldEntityComponent = createEntityComponent(
  radioGridFieldEntity,
  function RadioGridFieldEntityComponent(props) {
    const id = useId();

    const [selected, setSelected] = useState<{ [key: string]: string }>({});

    const handleChange = (row: string, column: string) => {
      setSelected({ ...selected, [row]: column });
    };
...
export function RadioGridFieldEntityAttributes() {
  return (
    <Flex row={false}>
